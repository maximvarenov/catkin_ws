...
class vtkViewerFrame(QtWidgets.QFrame):
    ...
    def updataVTKdraw(self):        
        ...
        self.actor.GetProperty().SetColor([176/255,75/255,75/255])
    ...
    def upgtdataVTKdraw(self):        
        ...
        self.gtactor.GetProperty().SetColor([253/255,249/255,238/255])
    ...
    ##----------------------------------------------------------------------------------------------------
    ## ICP class:   FusionTrack: create new Sphere Actor for n43650ew FT point
    ##
    ##
    ...
    def initialAlignment(self):
        ...
        # stlWriter.SetFileName("./gt_2.stl")
    ...
    def IcpRegistration(self):
        ...
        # convert FT point cloud to VTK point 
        print ("========= ICP python registrationb ========") 
 
 
        # define ICP transform
        icp = vtk.vtkIterativeClosestPointTransform()
        icp.SetSource(self.LmRegPoly)
        icp.SetTarget(self.GTModelData.gtData)
        #icp.GetLandmarkTransform().SetModeToAffine()
        icp.GetLandmarkTransform().SetModeToRigidBody()
        #icp.DebugOn()
        icp.SetMaximumNumberOfIterations(10)
        icp.SetMaximumNumberOfLandmarks(maxiOfLandmarks)
        icp.StartByMatchingCentroidsOn()
        icp.Modified()
        icp.Update()
        
        # apply ICP transform to Landmark result for the refine registration
        icpTransformFilter = vtk.vtkTransformPolyDataFilter()
        icpTransformFilter.SetInputData(self.LmRegPoly)
        icpTransformFilter.SetTransform(icp)
        icpTransformFilter.Update()
        icpRegPoly = icpTransformFilter.GetOutput()

        self.clearSphereActorlist_STL()   
        self.updataVTK_Redraw(icpRegPoly)

          

    ##------------------------------------------------------------------------------
    ## ICP class:  ICP python
    ##
    ##
    def ICP_python(self):
        ...
        self.lastTime_distances = CM.get_distance(source, target) 
        print(self.lastTime_distances)
        ...
        # stlWriter.SetInputData(self.LmRegPoly) 
        # stlWriter.Write()
