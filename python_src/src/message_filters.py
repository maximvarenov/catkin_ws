#!/home/ruixuan/anaconda3/envs/tf_gpu/bin/python3
...
class Cache(SimpleFilter):
    ...
    def getInterval(self, from_stamp, to_stamp):
        ...
        return [msg for (msg, time) in list(zip(self.cache_msgs, self.cache_times))
                if from_stamp <= time <= to_stamp]

    def getElemAfterTime(self, stamp):
        """Return the oldest element after or equal the passed time stamp."""
        newer = [msg for (msg, time) in list(zip(self.cache_msgs, self.cache_times))
                 if time >= stamp]
        if not newer:
            return None
        return newer[0]
    ...
    def getElemBeforeTime(self, stamp):
        ...
        older = [msg for (msg, time) in list(zip(self.cache_msgs, self.cache_times))
                 if time <= stamp]
        if not older:
            return None
        return older[-1]
