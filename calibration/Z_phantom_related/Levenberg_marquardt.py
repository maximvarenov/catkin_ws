...
class Levenberg_marquardt():
    ...
    def generate_data(self,params,num_data):
        x = np.array(np.linspace(0,10,num_data)).reshape(num_data,1)
        # x1 = np.array(np.linspace(0,10,num_data)).reshape(num_data,1)
        # x = np.concatenate((x,x1),axis=1)

        mid,sigma = 0,5
        y =  self.my_Func(params,x) + np.random.normal(mid, sigma, num_data).reshape(num_data,1)
        return x,y
    ...
    def cal_deriv(self, params,input_data,param_index):
        ...
        data_est_output1 = self.my_Func(params1,input_data)
        data_est_output2 = self.my_Func(params2,input_data)
    ...
    def cal_Jacobian(self,params,input_data):
        ...
        # print("\n generate_output_data: "+ str(J.shape))
    ...
    def cal_residual(self,params,input_data,output_data):
        data_est_output = self.my_Func(params,input_data)
        output_data = output_data.reshape((output_data.shape[0],1))
        data_est_output = data_est_output.reshape((data_est_output.shape[0],1))
    ...
    #LM algorithm
    def LM(self,num_iter,params,input_data,output_data):
        ...
        # print("\n residual.shape"+str(residual.shape))
        ...
        while((not stop) and (k<num_iter)):
            ...
            while(1):
                ...
                if(np.linalg.norm(step) <= self.threshold_step):
                    ...
                    new_residual = self.cal_residual(new_params,input_data,output_data)#get new residual using new params
                    # print("\n new_residual: "+ str(new_residual.shape))
                    rou = (np.linalg.norm(residual)**2 - np.linalg.norm(new_residual)**2) / (step.T.dot(u*step+g))

                    if rou > 0:
                        params = new_params
                        residual = new_residual
                        self.residual_memory.append(np.linalg.norm(residual)**2)
                        #print (np.linalg.norm(new_residual)**2)
                        Jacobian = self.cal_Jacobian(params,input_data)#recalculating Jacobian matrix with new params
                        A = Jacobian.T.dot(Jacobian)#recalculating A
                        g = Jacobian.T.dot(residual)#recalculating gradient g
                        stop = (np.linalg.norm(g, ord=np.inf) <= self.threshold_stop) or (np.linalg.norm(residual)**2 <= self.threshold_residual)
...
#         output[i][0] = a*np.exp(b*input_data[i]) + c*np.exp(d*input_data[i])
...
def generate_data(params,num_data):
    ...
    # x1 = np.array(np.linspace(0,10,num_data)).reshape(num_data,1)       # 产生包含噪声的数据
    # x = np.concatenate((x,x1),axis=1)
...
def main():
    ...
    # x = np.array(np.linspace(0,10,num_data)).reshape(num_data,1)       # create data with noise
    # x1 = np.array(np.linspace(0,10,num_data)).reshape(num_data,1)       # create data with noise
    # x = np.concatenate((x,x1),axis=1)
    x = np.arange(0,100)*0.1
    plt.plot(x,a_est*np.exp(b_est*x) + c_est*np.exp(d_est*x),'r',lw=1.0)
